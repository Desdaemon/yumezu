...
//!
//! A world's cell is its index among the worlds the app draws, which the app derives from the
//! atlas it loads, so there is no manifest to keep in step. What the two sides share is [`CELL`],
//! which the app checks the atlas against on load, and which worlds are left out -- the secrets,
//! which the app drops on reading the dump and this has to drop in the same place, or every cell
//! after the first of them is off by one.

use std::io::Write;
use std::path::{Path, PathBuf};

/// 4:3, which nearly every world image already is, so the crop that fits an image to a cell
/// usually takes nothing off it.
///
...
///
/// Packed into the last cell of the grid rather than one counted off the worlds, so the app finds
/// it from the atlas's shape alone and neither side carries a number the other could get wrong.
...
fn main() {
    ...
    // picture rather than from its cell, there being no sharper version to switch to and a cell of
    // it being this line art run through the atlas's jpeg. The cell is still packed, for the one
    // place that reads the atlas alone -- the sidebar's catalog.
    //
    // Decoded and written out again rather than copied: the wiki serves this one as an indexed
    // png, which the app's own decoder does not read as colour -- it arrives as one channel, which
    // draws the black background red. Every other picture goes through the same conversion on its
    // way into the atlas, so this is only that conversion kept.
    ...
    // Square-ish, so neither dimension runs far ahead of the other and into a driver's texture
    // size limit. Sized for the worlds *and* the placeholder, so there is always a last cell left
    // over for it.
    ...
    for (world, bytes) in images.iter().enumerate() {
        let Some(thumbnail) = bytes.as_ref().and_then(|bytes| thumbnail(bytes)) else {
            // Left black: the app draws the world's plate either way, so a world whose image the
            // wiki no longer serves is a node without a picture rather than a hole in the atlas.
            continue;
        };
    ...
    // JPEG, at a fifth of what the same atlas costs as a PNG: the app fetches it over the network,
    // and a thumbnail this small has no detail to lose that survives being drawn a few tens of
    // pixels wide. Its 8x8 blocks divide both sides of a cell, so the artefacts it does introduce
    // stay inside the world they belong to.
...
/// Cropped to the cell's shape about its centre, then scaled. Cropping rather than letterboxing,
/// because a thumbnail this small has no room to spend on bars and the middle of a screenshot is
...
/// In world order, from the cache where already there and downloaded where not. `None` for a world
/// whose image cannot be had at all.
